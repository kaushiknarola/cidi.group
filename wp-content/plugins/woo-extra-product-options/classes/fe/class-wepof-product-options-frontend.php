...
class WEPOF_Product_Options_Frontend {
	...
	***************************************************/
	...
	public function woo_product_add_to_cart_text($text, $product){
		if($this->is_modify_product_add_to_cart_link($product)){
			$text = apply_filters('thwepof_loop_add_to_cart_link_text', 'Select options');
			...
			if($validators && !empty($value)){
				foreach($validators as $validator){
					switch($validator) {
						case 'number' :
							if(!is_numeric($value)){
								THWEPOF_Utils::wcpf_add_error('<strong>'.esc_html($field_label).'</strong> '. sprintf(THWEPOF_Utils::t('%s is not a valid number.'), $value));
								$valid = false;
							}
							break;
						...
						case 'email' :
							if(!is_email($value)){
								THWEPOF_Utils::wcpf_add_error('<strong>'.esc_html($field_label).'</strong> '. sprintf(THWEPOF_Utils::t('%s is not a valid email address.'), $value));
								$valid = false;
							}
							break;
					}
	...
	*********************************************************/
	...
	public function woo_get_item_data($item_data, $cart_item = null) {
		if(apply_filters('thwepof_display_custom_cart_item_meta', true)){
	...
	public function woo_order_item_get_formatted_meta_data( $formatted_meta, $order_item){
		$show_in_order = apply_filters('thwepof_show_order_item_custom_meta', true);
