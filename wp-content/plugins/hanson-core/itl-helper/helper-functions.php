...
if ( function_exists( 'wpcf7' ) ) {
    function itl_select_contact_form(){
...
function itl_get_terms($taxonomy) {
	global $wpdb;
	$term_coll = array();
...
function itl_get_chosen_terms($terms_query) {
	$chosen_terms = array();
	$taxonomies = array();
...
function itl_entry_terms_list($taxonomy = 'category', $separator = ', ', $before = ' ', $after = ' ') {
	global $post;
...
function itl_get_taxonomy_info($taxonomies) {
	$output = '';
...
function itl_entry_published($format = null) {
	if (empty($format))
...
function itl_get_column_class($column_size = 3, $no_margin = false) {
	$style_class = 'lae-threecol';
	$no_margin = itl_to_boolean($no_margin); // make sure it is not string
	$column_styles = array(
		1 => 'lae-twelvecol',
		2 => 'lae-sixcol',
		3 => 'lae-fourcol',
...
function itl_get_taxonomies_map() {
	$map = array();
 ...
 */
function itl_color_luminance($hex, $percent) {
	// validate hex string
...
function itl_get_option($option_name, $default = null) {
	$settings = get_option('itl_settings');
...
function itl_update_option($option_name, $option_value) {
	$settings = get_option('itl_settings');
	if (empty($settings))
		$settings = array();
	$settings[$option_name] = $option_value;
	update_option('itl_settings', $settings);
}
 ...
 */
function itl_update_options($setting_data) {
	$settings = get_option('itl_settings');
	if (empty($settings))
		$settings = array();
...
function itl_get_sysinfo() {
	...
	// Theme info
	$plugin = get_plugin_data(itl_PLUGIN_FILE);
...
function itl_get_taxonomy_terms_filter($taxonomies, $chosen_terms = array()) {
	$output = '';
	$terms = array();
	...
	if (!empty($terms)) {
		$output .= '<div class="itl-portfolio-nav">';
			$output .= '<ul>';
				$output .= '<li class="active">'.esc_html__( 'All', 'hanson-core' ).'</li>';
				foreach ($terms as $term){
					$output .= '<li data-filter=".'.esc_attr($term->slug).'">'.esc_html($term->name).'</li>';
				}
			$output .= '</ul>';
		$output .= '</div>';
	}
