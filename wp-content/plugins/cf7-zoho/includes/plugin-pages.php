...
if( !class_exists( 'vxcf_zoho_pages' ) ) {
...
* @since       1.0.0
*/
class vxcf_zoho_pages   extends vxcf_zoho{
    public $ajax=false;
...
public function add_bulk_send_crm_callcenter($list){ 
 $list['send_zoho']=__('Send to Zoho','contact-form-zoho-crm');
  return $list;  
}
...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo $this->id ?>_send" value="yes"> <?php _e('Send to Zoho','contact-form-zoho-crm') ?></label>
  </div>
  <?php  
}
    /**
...
$lead['__vx_id']=$id;
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'contact-form-zoho-crm') . '</a>' );
        }
        return $links;
   }
  /**
  ...
  */
  ...
  // Adding submenu if user has access
        $page_title =__('Zoho for Contact Form','contact-form-zoho-crm');
        $menu_title =__('Zoho','contact-form-zoho-crm');
...
if(isset($_REQUEST[$this->id.'_tab_action']) && $_REQUEST[$this->id.'_tab_action']=="get_token"){
  ...
  $msg=__('You do not have permissions to add token','contact-form-zoho-crm');
...
$class='';
 if($res){
       $msg=__('Account Deleted Successfully','contact-form-zoho-crm');
  $class='updated';   
 }else{
       $msg=__('Error While Removing Account','contact-form-zoho-crm');
  $class='error';      
 }
...
die();
  ...
  $msg=__('You do not have permissions to export logs','contact-form-zoho-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  ...
  $fields=array(); $field_titles=array("#",__('Status','contact-form-zoho-crm'),__('Zoho ID','contact-form-zoho-crm') ,__('Entry ID','contact-form-zoho-crm'),__('Description','contact-form-zoho-crm'),__('Time','contact-form-zoho-crm'));
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','contact-form-zoho-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  $clear=$data->clear_logs();
       ...
       $msg=__('Error While Clearing Zoho Logs','contact-form-zoho-crm');
      $level="error";
      if(!empty($clear)){
      $msg=__('Zoho Logs Cleared Successfully','contact-form-zoho-crm');   
      $level="updated";
      }
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  __('Enter feed name of your choice.', 'contact-form-zoho-crm'),
  'vx_sel_object' => __('Select the Object to Create when a Form is Submitted.', 'contact-form-zoho-crm'),
   'vx_sel_account' =>__('Select the Zoho account you would like to export entries to.', 'contact-form-zoho-crm'),
  'vx_sel_form' => __('Select the Contact Form you would like to integrate with Zoho. Contacts generated by this form will be automatically added to your Zoho account.', 'contact-form-zoho-crm'),
  
  'vx_map_fields' => __('Associate your Zoho fields to the appropriate Contact Form fields.', 'contact-form-zoho-crm'),
  
  'vx_optin_condition' =>__('When the opt-in condition is enabled, form submissions will only be exported to Zoho when the condition is met. When disabled all form submissions will be exported.', 'contact-form-zoho-crm'),
  
  'vx_manual_export' => __('If you do not want all entries sent to Zoho, but only specific, approved entries, check this box. To manually send an entry to Zoho, go to Entries, choose the entry you would like to send to Zoho, and then click the "Send to Zoho" button.', 'contact-form-zoho-crm'),
  
    'vx_entry_notes' => __('Enable this option if you want to synchronize Contact Form entry notes to Zoho Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Zoho Object selected in the feed.', 'contact-form-zoho-crm'),
    
      'vx_primary_key' => __('Which field should be used to update existing objects?', 'contact-form-zoho-crm'),
      
  'vx_web' => sprintf(__('Web-to-Lead is available for all Zoho Editions. If you are not sure if your Zoho Edition supports the API, you should use Web-to-Lead. Editions that do not support the Zoho API: %s 1: Personal Edition %s 2: Group Edition %s 3: Professional Edition %s Note: You can purchase API access for a Professional Edition', 'contact-form-zoho-crm'),'<br/>','<br/>','<br/>','<br/>'),
  'vx_api' => sprintf(__('The API features are more powerful than Web-to-Lead. You can create different object types, as well as other advanced features. If you have any of the following Zoho Editions, you can use the included API Add-on: %s 1: Enterprise Edition %s 2: Unlimited Edition %s 3: Developer Edition %s 4: Professional Edition - Requires API Upgrade', 'contact-form-zoho-crm'),'<br/>','<br/>','<br/>','<br/>','<br/>'),
  
  'vx_custom_app'=>__('This option is for advanced users who want to override default Zoho App.','contact-form-zoho-crm'),
  
  'vx_disable_logs'=>__('When an order is sent to Zoho we store that order information in the database and show it in the Zoho Log. Check this box if you do not want to save the exported order information in the logs.','contact-form-zoho-crm'),
  
  'vx_line_items'=>__('Create a Zoho Order product for each Woocommrce Cart item.','contact-form-zoho-crm'),
  
  'vx_price_books'=>__('Get PriceBooks list from zoho.','contact-form-zoho-crm'),
  'vx_camps'=>__('Get Campaigns and Status list from zoho.','contact-form-zoho-crm'),
  
  'vx_sel_price_book'=>__('Which Pricebook should be searched for product','contact-form-zoho-crm'),
  'vx_sel_camp'=>__('Which Campaign should be assigned to this object.','contact-form-zoho-crm'),
  'vx_sel_status'=>__('What should be Member Status.','contact-form-zoho-crm'),
  
  'vx_pro_desc'=>__('A new product will be created in selected Pricebook. You can add a description for new products created by this plugin.','contact-form-zoho-crm'),
  
   'vx_assign_account'=>__('Enable this option if you want to assign an account this object.','contact-form-zoho-crm'),
   'vx_sel_account'=>__('Object created by this feed will be assigned to the selected Account.','contact-form-zoho-crm'),
   
      'vx_assign_vendor'=>__('Enable this option if you want to assign an vendor this object.','contact-form-zoho-crm'),
   'vx_sel_vendor'=>__('Object created by this feed will be assigned to the selected Vendor.','contact-form-zoho-crm'),
   
   
      'vx_assign_contract'=>__('Enable this option , if you want to assign a Contract to this object','contact-form-zoho-crm'),
   'vx_sel_contract'=>__('Select Contract feed. Contract created by this feed will be assigned to this object','contact-form-zoho-crm'),
   
   'vx_camp_check'=>__('If enabled, Lead/Contact will be added to selected Campaign','contact-form-zoho-crm'),
   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','contact-form-zoho-crm'),
   'vx_owners'=>__('Get Users list from Zoho','contact-form-zoho-crm'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Zoho Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Zoho Object selected in the feed.','contact-form-zoho-crm'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','contact-form-zoho-crm'),
   
       'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note.', 'contact-form-zoho-crm'),
   'vx_note_fields'=>__('Select fields which you want to send as a note', 'contact-form-zoho-crm'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry', 'contact-form-zoho-crm')
...
<div class="table_head_i"><?php _e('Capsule', 'contact-form-zoho-crm'); ?></div>  
<div class="vx_group">
...
echo $this->format_log_msg($log);
 ...
 $msg=__('You do not have permissions for this action','contact-form-zoho-crm');
...
if( $this->has_feed($form_id)) { 
 $boxes['zoho_crm']=array('title'=>'<i class="fa fa-plug"></i> '.__('Zoho', 'contact-form-zoho-crm'),'callback'=>array($this,'meta_box_html'));  
} return $boxes; 
}
...
public function has_feed($form_id) {
 ...
 if(!empty($info['meta']) ){
  ...
  switch($action){
    ...
    if(is_array($camps)){
    $res['status']="ok";
    $data['crm_sel_camp']=$this->gen_select($camps,$status_sel,__('Select Campaign','contact-form-zoho-crm')); 
    }else{
     $res['error']=$camps;   
    }
$meta['campaigns']=$camps; 
  ...
  case"refresh_users":
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','contact-form-zoho-crm'));
  ...
  $res['data']=$data;   
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_layout']=$this->gen_select($users,'',__('Select Layout','contact-form-zoho-crm'));
    $meta['layouts']=$users;   
    }else{
     $res['error']=$users;   
    }
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','contact-form-zoho-crm');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  public  function mapping_page(){
      ...
      wp_enqueue_style('vxc-tooltip');
         $tabs=array('feeds'=>__('Zoho Feeds','contact-form-zoho-crm') , 'logs'=>__('Zoho Log','contact-form-zoho-crm') , 'accounts'=>__('Zoho Accounts','contact-form-zoho-crm'), 'settings'=>__('Settings','contact-form-zoho-crm'));
  ...
  $(this).text('<?php _e('Show Key','contact-form-zoho-crm') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php _e('Hide Key','contact-form-zoho-crm') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  _e('You do not have permissions to access this page','contact-form-zoho-crm');    
  return;
  }
 ...
 $id = absint($this->post("action_argument"));
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'contact-form-zoho-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'contact-form-zoho-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','contact-form-zoho-crm'),$count));  
    }
  ...
  if(!empty($entry) && is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','contact-form-zoho-crm'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','contact-form-zoho-crm'),"delete"=>__('Delete','contact-form-zoho-crm'),
  'send_to_crm_bulk'=>__('Send to Zoho','contact-form-zoho-crm'),'send_to_crm_bulk_force'=>__('Force Send to Zoho - Ignore filters','contact-form-zoho-crm'));
  $base_url=$this->get_base_url();
   $objects=get_option($this->id.'_meta',array()); 

   $objects=$this->post('objects',$objects);
      $statuses=array("1"=>__("Created",'contact-form-zoho-crm'),"2"=>__("Updated",'contact-form-zoho-crm'),"error"=>__("Failed",'contact-form-zoho-crm'),"4"=>__("Filtered",'contact-form-zoho-crm'),"5"=>__("Deleted",'contact-form-zoho-crm')); 
...
//
  ...
  $fields=$api->get_crm_fields($module); 
     ...
     $temp_fields=array(); $phones=array('Phone','AsstPhone','OtherPhone','HomePhone','Tag','tags','vx_attachments');
    foreach($fields as $k=>$v){
        if(empty($v['custom']) && !in_array($k,$phones)){ 
       $temp_fields[$k]=$v;     
        }
    }
   $fields= $temp_fields;
 ...
 $fields=$this->post('fields',$feed_meta); 
  ...
  $skip_fields=array();
    $show_account=false;
    $show_vender=false;
  $req_fields=array('Last_Name','Company_Name');
    
    $n=0;
  foreach($fields as $k=>$v){
         $n++;
      if(in_array($k,$req_fields)){
   $map_fields[$k]=$v;       
      } 
}
//mapping fields
foreach($map as $field_k=>$field_v){
  ...
  $sel_fields=array(""=>__("Standard Field",'contact-form-zoho-crm'),"value"=>__("Custom Value",'contact-form-zoho-crm'));
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = __('Zoho','contact-form-zoho-crm');
        return $plugins;
    }
  /**
...
$this->ajax=true;
  ...
  $msg=__("Please Choose Object",'contact-form-zoho-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-zoho-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'contact-form-zoho-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-zoho-crm');
  }
  if($msg !=""){
...
$field_options="<option>".__("Select Object",'contact-form-zoho-crm')."</option>"; 
  if(is_array($objects)){
...
die();
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','contact-form-zoho-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Contact Form Zoho Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-zoho-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-zoho-crm'));
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  _e('You do not have permissions to save settings','contact-form-zoho-crm');
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-zoho-crm'));
  $valid_email=true;
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','contact-form-zoho-crm'));
  }
   $info=$this->get_info($id); 
   $crm=$this->post('crm');    
...
//  $force_check=true;
  ...
  if($con_class != "updated" ){
      $msg=__('Connection to Zoho is NOT Working','contact-form-zoho-crm');  
  }else{
     $msg=__('Connection to Zoho is Working','contact-form-zoho-crm');   
  }
  $title=__('Test Connection: ','contact-form-zoho-crm');
...
$feed= $this->data->get_feed($fid); 
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  _e('You do not have permissions to edit/save feed','contact-form-zoho-crm'); 
  return;
  }
  //
...
if(is_array($feed_update) && is_array($feed)){
  ...
  if($is_valid){
      ...
      $msgs['save']=array('class'=>'updated','msg'=>sprintf(__("Feed Updated. %sback to list%s", 'contact-form-zoho-crm'), '<a href="'.$feed_link.'">', "</a>"));
  }
  else{
  $msgs['save']=array('class'=>'error','msg'=>__("Feed could not be updated. Please enter all required information below.", 'contact-form-zoho-crm'));
...
if(is_array($tags)){
  ...
  $fields['__vx_id']=array('id'=>'__vx_id','label'=>__('Entry ID','contact-form-zoho-crm')); 
$fields['_vx_form_id']=array('id'=>'_vx_form_id','label'=>__('Form ID','contact-form-zoho-crm')); 
$fields['_vx_form_name']=array('id'=>'_vx_form_title','label'=>__('Form Title','contact-form-zoho-crm')); 

$fields['_vx_url']=array('id'=>'_vx_url','label'=>__('Page URL','contact-form-zoho-crm')); 
$fields['_vx_created']=array('id'=>'_vx_created','label'=>__('Entry Created','contact-form-zoho-crm')); 
$fields['_vx_updated']=array('id'=>'_vx_updated','label'=>__('Entry Updated','contact-form-zoho-crm'));

$fields=array('cf'=>array("title"=>__('Contact Form Fields','contact-form-zoho-crm'),"fields"=>$fields));
 ...
 if($this->do_actions()){ 
  ...
  $fields['feeds']=array("title"=>__('ID from other Feeds','contact-form-zoho-crm'),"fields"=>$feeds);
  }
 }
  $this->fields=$fields;
...
if(is_array($fields_arr['fields'])){
  ...
  */
  ...
  $modules=array(""=>__("Select Object",'contact-form-zoho-crm'));
  ...
  */
  public function validate_api($info,$force_check=false){
 ...
 /* if(!$force_check && $api_check<$time){ //check validity period in settings tab
  ...
  $msg=__( 'Successfully Connected to Zoho','contact-form-zoho-crm' );
