<?php
/**
* Plugin Name: Contact Form 7 Zoho
* Description: Integrates Contact Form 7 and <a href="https://wordpress.org/plugins/contact-form-entries/">Contact Form Entries Plugin</a> with Zoho allowing form submissions to be automatically sent to your Zoho account 
* Version: 1.1.2
...
class vxcf_zoho {
 ...
 public  $url = "https://www.crmperks.com";
  ...
  public static $title='Contact Form Zoho Plugin';  
  public static $path = ''; 
  public static $slug = "";
...
//load_plugin_textdomain('contact-form-zoho-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
...
$val=$submission->get_posted_data($name);
   ...
   if( !empty($val) && $v['basetype'] == 'mfile' && function_exists('dnd_get_upload_dir') ){
...
if(!isset($lead[$name])){  $lead[$name]=wp_unslash($val);  }
...
$this->push($entry,$form,'',false);    
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo $id ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Zoho Plugin','contact-form-zoho-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php _e('Dismiss Notice','contact-form-zoho-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Zoho Plugin','contact-form-zoho-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'contact-form-zoho-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Zoho Plugin','contact-form-zoho-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'contact-form-zoho-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
$value=trim(implode(' ',$value));
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'contact-form-zoho-crm');
  ...
  if(!empty($feed['crm_id'])&& !empty($feed['object']) && !empty($feed['status'])){
      if(in_array($feed['object'],array('Note','Task'))){
    $link=$feed['crm_id'];      
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=sprintf(__("%s Deleted",'contact-form-zoho-crm'),$feed['object']);   
  }
  }else if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'contact-form-zoho-crm'),$feed['object']); 
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : __("Unknown Error",'contact-form-zoho-crm');
  }
  $title=__("Failed",'contact-form-zoho-crm');   
  if($feed['status'] == 1){
  $title=__("Created",'contact-form-zoho-crm');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'contact-form-zoho-crm');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'contact-form-zoho-crm');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'contact-form-zoho-crm');   
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.$this->id.'_update" id="'.$this->id.'_update" value="1" /><label for="'.$this->id.'_update" title="%s">%s</label><br /><br />', __('Create or update this entry in Zoho. The fields will be mapped according to the form feed settings.', 'contact-form-zoho-crm'), __('Send to Zoho', 'contact-form-zoho-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.$this->id.'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Zoho. The fields will be mapped according to the form feed settings.','contact-form-zoho-crm').'" value="'.__('Send to Zoho', 'contact-form-zoho-crm').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Zoho Logs','contact-form-zoho-crm').'" href="'.$log_url.'">'.__('Zoho Logs','contact-form-zoho-crm').'</a>';
...
return  $var;
  ...
  */
  ...
  $status=0;
  if(class_exists('vxcf_zoho_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/contact-form-zoho-crm/contact-form-zoho-crm.php')) {
  $status=2;
  } 
  return $status;
  }
 ...
 $offset = (int) get_option('gmt_offset');
  ...
  /**
  * Decrypts Values
  * @param array $info Zoho encrypted API info 
  ...
  $e=1000; $k=1;
    foreach($feeds as $feed){
          $data=isset($feed['data']) ? json_decode($feed['data'],true) : array(); 
           ...
           $feed['meta']=$meta;
           $feed['data']=$data;
   if(!empty($data['account_check']) || !empty($data['vendor_check'])){
     $feeds_meta[$e++]=$feed; 
  }else{
  $feeds_meta[$k++]=$feed; 
  }
...
// add note and save related extra info
 if( $event == 'add_note' && !empty(self::$note)){
    $temp=array('Title'=>array('label'=>'Note Title','value'=>self::$note['title']),'Body'=>array('label'=>'Note Content','value'=>self::$note['body']),'ParentId'=>array('label'=>'entityId', 'value'=> $feed['crm_id']));  
$parent_id=self::$note['id']; 
...
}else if($field == '_vx_form_id' && isset($form['id'])){
  $value=$form['id'];   
...
if( $value !== false ){
  ...
  if(!empty($data['owner']) && !empty($data['user']) ){
   $data['user']=apply_filters('vx_assigned_user_id',$data['user'],$this->id,$feed['id'],$entry);   
  }  
  //add note 
...
$info=$this->get_info($account); 
 if(is_array($meta) && is_array($data)){
      $feed=array_merge($feed,$meta,$data);
  }
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Zoho (%s) with ID # %s .', 'contact-form-zoho-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Zoho (%s) with ID # %s . View entry at %s', 'contact-form-zoho-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Zoho', 'contact-form-zoho-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Zoho (%s) with ID # %s . View entry at %s', 'contact-form-zoho-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Zoho (%s) with ID # %s .', 'contact-form-zoho-crm'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Zoho', 'contact-form-zoho-crm');
          }else{
     $screen_msg=__( 'Entry updated in Zoho', 'contact-form-zoho-crm');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'contact-form-zoho-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Zoho. Entry not sent! Check the Entry Notes below for more details.' , 'contact-form-zoho-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Zoho. Entry not sent' , 'contact-form-zoho-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'contact-form-zoho-crm'),$feed['object']);
  if($this->post('error',$res)!=""){
      $error= is_array($res['error']) ? json_encode($res['error']) : $res['error'];
  $msg.=" ($error)";
  
  $_REQUEST['VXGZohoError']=$msg; //front end form error for admin only
  }   
  if(!$is_admin){
      $info['msg']=$msg;
...
//  } 
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','contact-form-zoho-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $subject="Error While Posting to Zoho";
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Zoho','contact-form-zoho-crm')." Error","info_title"=>"More Detail","info"=>$detail);
...
$vxcf_zoho=new vxcf_zoho();
$vxcf_zoho->instance();
$vx_cf['vxcf_zoho']='vxcf_zoho';
endif;
